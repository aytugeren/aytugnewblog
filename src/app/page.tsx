...
export default async function HomePage() {
  ...
  type Project = {
    projectName: string;
    projectTechnologies?: string | string[]; // comma separated or array
  ...
  try {
    ...
    if (postsRes.ok) {
      postsFromApi = await postsRes.json();
    }
  } catch (error) {
  ...
  const posts = (Array.isArray(postsFromApi) && postsFromApi.length > 0)
    ? postsFromApi
    : (data.posts ?? []);
      ...
      <Entrance3D>
        <div className="relative">
          ...
          <main>
              ...
              <div className="grid items-center gap-12 md:grid-cols-2">
                <div>
                  <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">
                    Aytuğ Y — Senior .NET &amp; Frontend Engineer
                  </h1>
            ...
            <Section title="Üzerinde Çalıştığım Projeler" description="Tamamlanma yüzdesi ile güncel durum">
              {ongoingProjects.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">Şu an paylaşılan bir proje yok.</p>
              )}
            </Section>
                ...
                {experiences.map((exp,key) => {
                  ...
                  return (
                    <TimelineItem
                      ...
                      role={exp.tag}
                      period={period}
                      achievements={exp.workDescription ? [exp.workDescription] : []}
                    />
                  );
                })}
              </ol>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                {projects.map((proj, idx) => {
                  ...
                  const tags = Array.isArray(proj.projectTechnologies)
                    ? proj.projectTechnologies.map((t) => String(t).trim()).filter(Boolean)
                    : Array.isArray(legacyTags)
                    ? legacyTags.map((t: any) => String(t).trim()).filter(Boolean)
                    : String((proj.projectTechnologies as any) ?? "")
                        .split(",")
                        .map((t) => t.trim())
                        .filter(Boolean);

                  return (
                    <ProjectCard key={idx} title={name} summary={summary} tags={tags} href={legacyLink as any} />
                  );
                })}
              </div>
            </Section>
