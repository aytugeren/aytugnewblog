    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button disabled={loading} onClick={saveAll} className="px-3 py-1 rounded bg-blue-600 text-white disabled:opacity-50">Tümünü Kaydet</button>
        </div>
      </div>
      ...
      <section className="border rounded p-4 space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          {data.HasCv && (
            <>
              ...
              <button type="button" onClick={downloadCv} className="text-sm px-2 py-1 border rounded">İndir</button>
            </>
          )}
        </div>
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={uploadCv} disabled={loading || !cvFile} className="px-3 py-1 rounded bg-blue-600 text-white disabled:opacity-50">Yükle</button>
          </div>
          <p className="text-xs text-gray-500">Dosya, site kökündeki <code>/cv.pdf</code> olarak kaydedilir.</p>
        </div>
      </section>
  ...
  return (
    <section className="border rounded p-4 space-y-3">
      ...
      <div className="flex gap-2">
        <input value={newCat} onChange={(e) => setNewCat(e.target.value)} placeholder="Kategori adı (örn. Languages)" className="border rounded px-2 py-1 flex-1 bg-transparent" />
        ...
        {value.map((e, i) => (
          <div key={i} className="border rounded p-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <input className="border rounded px-2 py-1 bg-transparent" placeholder="Şirket" value={e.CompanyName} onChange={(ev) => update(i, { CompanyName: ev.target.value })} />
              <input className="border rounded px-2 py-1 bg-transparent" placeholder="Etiket (örn. Fullstack)" value={e.Tag} onChange={(ev) => update(i, { Tag: ev.target.value })} />
              <input className="border rounded px-2 py-1 bg-transparent" placeholder="Başlangıç (YYYY-MM)" value={e.BeginDate} onChange={(ev) => update(i, { BeginDate: ev.target.value })} />
              <input className="border rounded px-2 py-1 bg-transparent" placeholder="Bitiş (YYYY-MM / Devam)" value={e.EndDate} onChange={(ev) => update(i, { EndDate: ev.target.value })} />
            </div>
            <textarea className="w-full border rounded px-2 py-1 bg-transparent" placeholder="Açıklama" value={e.WorkDescription} onChange={(ev) => update(i, { WorkDescription: ev.target.value })} />
            <div className="text-right">
              <button onClick={() => remove(i)} className="text-sm px-2 py-1 border rounded text-red-600">Sil</button>
            </div>
          </div>
        ))}
        {value.length === 0 && <p className="text-sm text-gray-500">Henüz deneyim eklenmedi.</p>}
      </div>
    </section>
  )
}
    ...
    <section className="border rounded p-4 space-y-3">
      ...
      <div className="space-y-3">
        {value.map((p, i) => (
          <div key={i} className="border rounded p-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <input className="border rounded px-2 py-1 bg-transparent" placeholder="Proje Adı" value={p.ProjectName} onChange={(ev) => update(i, { ProjectName: ev.target.value })} />
              ...
              <input className="border rounded px-2 py-1 bg-transparent" placeholder="Bağlantı (opsiyonel)" value={p.ProjectLink ?? ''} onChange={(ev) => update(i, { ProjectLink: ev.target.value })} />
            </div>
            <textarea className="w-full border rounded px-2 py-1 bg-transparent" placeholder="Açıklama" value={p.ProductDescription} onChange={(ev) => update(i, { ProductDescription: ev.target.value })} />
            <div className="text-right">
              <button onClick={() => remove(i)} className="text-sm px-2 py-1 border rounded text-red-600">Sil</button>
            </div>
          </div>
  ...
  return (
    <section className="border rounded p-4 space-y-3">
      ...
      <div className="space-y-3">
        ...
        {value.length === 0 && <p className="text-sm text-gray-500">Henüz öğe eklenmedi.</p>}
      </div>
    </section>
  )
}
